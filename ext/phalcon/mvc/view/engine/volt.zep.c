...
PHP_METHOD(Phalcon_Mvc_View_Engine_Volt, getCompiler) {
	...
	if (Z_TYPE_P(compiler) != IS_OBJECT) {
		...
		ZEPHIR_CALL_METHOD(NULL, compiler, "__construct", NULL, 359, _0$$3);
		...
		if (Z_TYPE_P(dependencyInjector) == IS_OBJECT) {
			ZEPHIR_CALL_METHOD(NULL, compiler, "setdi", NULL, 360, dependencyInjector);
			zephir_check_call_status();
		}
		ZEPHIR_OBS_VAR(options);
		zephir_read_property_this(&options, this_ptr, SL("_options"), PH_NOISY_CC);
		if (Z_TYPE_P(options) == IS_ARRAY) {
			ZEPHIR_CALL_METHOD(NULL, compiler, "setoptions", NULL, 361, options);
			zephir_check_call_status();
		}
...
PHP_METHOD(Phalcon_Mvc_View_Engine_Volt, render) {
	...
	if (Z_TYPE_P(params) == IS_ARRAY) {
		zephir_is_iterable(params, &_1$$4, &_0$$4, 0, 0, "phalcon/mvc/view/engine/volt.zep", 118);
		for (
...
PHP_METHOD(Phalcon_Mvc_View_Engine_Volt, isIncluded) {
	...
	if (Z_TYPE_P(haystack) == IS_STRING) {
		if ((zephir_function_exists_ex(SS("mb_strpos") TSRMLS_CC) == SUCCESS)) {
			ZEPHIR_CALL_FUNCTION(&_0$$5, "mb_strpos", NULL, 362, haystack, needle);
			zephir_check_call_status();
...
PHP_METHOD(Phalcon_Mvc_View_Engine_Volt, convertEncoding) {
	...
	if (_0) {
		ZEPHIR_RETURN_CALL_FUNCTION("utf8_encode", NULL, 363, text);
		zephir_check_call_status();
		RETURN_MM();
	}
	...
	if (_1) {
		ZEPHIR_RETURN_CALL_FUNCTION("utf8_decode", NULL, 364, text);
		zephir_check_call_status();
		RETURN_MM();
	}
	...
	if ((zephir_function_exists_ex(SS("iconv") TSRMLS_CC) == SUCCESS)) {
		ZEPHIR_RETURN_CALL_FUNCTION("iconv", NULL, 365, from, to, text);
		zephir_check_call_status();
		RETURN_MM();
	}
...
PHP_METHOD(Phalcon_Mvc_View_Engine_Volt, slice) {
	...
	if (Z_TYPE_P(value) == IS_ARRAY) {
		...
		ZEPHIR_RETURN_CALL_FUNCTION("array_slice", NULL, 366, value, &_5$$9, length);
		zephir_check_call_status();
		RETURN_MM();
	}
	if ((zephir_function_exists_ex(SS("mb_substr") TSRMLS_CC) == SUCCESS)) {
		if (Z_TYPE_P(length) != IS_NULL) {
			...
			ZEPHIR_RETURN_CALL_FUNCTION("mb_substr", &_7, 367, value, &_6$$11, length);
			zephir_check_call_status();
			RETURN_MM();
		}
		ZEPHIR_SINIT_VAR(_8$$10);
		ZVAL_LONG(&_8$$10, start);
		ZEPHIR_RETURN_CALL_FUNCTION("mb_substr", &_7, 367, value, &_8$$10);
		zephir_check_call_status();
		RETURN_MM();
	}
	if (Z_TYPE_P(length) != IS_NULL) {
...
PHP_METHOD(Phalcon_Mvc_View_Engine_Volt, sort) {
	...
	ZEPHIR_CALL_FUNCTION(NULL, "asort", NULL, 368, value);
	ZEPHIR_UNREF(value);
	zephir_check_call_status();
	RETURN_CTOR(value);
...
PHP_METHOD(Phalcon_Mvc_View_Engine_Volt, callMacro) {
	...
	ZEPHIR_RETURN_CALL_FUNCTION("call_user_func", NULL, 369, macro, arguments);
	zephir_check_call_status();
	RETURN_MM();
