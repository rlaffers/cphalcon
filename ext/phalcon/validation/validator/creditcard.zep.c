...
PHP_METHOD(Phalcon_Validation_Validator_CreditCard, validate) {
	...
	ZEPHIR_CALL_METHOD(&valid, this_ptr, "verifybyluhnalgorithm", NULL, 434, value);
	zephir_check_call_status();
	if (!(zephir_is_true(valid))) {
		...
		ZEPHIR_CALL_FUNCTION(&_2$$3, "strtr", NULL, 54, message, replacePairs);
		zephir_check_call_status();
		ZEPHIR_INIT_VAR(_3$$3);
		ZVAL_STRING(_3$$3, "CreditCard", ZEPHIR_TEMP_PARAM_COPY);
		ZEPHIR_CALL_METHOD(NULL, _0$$3, "__construct", NULL, 430, _2$$3, field, _3$$3);
...
PHP_METHOD(Phalcon_Validation_Validator_CreditCard, verifyByLuhnAlgorithm) {
	...
	ZVAL_STRING(hash, "", 1);
	ZEPHIR_CALL_FUNCTION(&_4, "array_reverse", NULL, 435, digits);
	zephir_check_call_status();
	...
	zephir_check_call_status();
	ZEPHIR_CALL_FUNCTION(&result, "array_sum", NULL, 436, _9);
	zephir_check_call_status();
